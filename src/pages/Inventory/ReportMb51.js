...
import { Input, Button, Modal, ModalBody, Spinner } from 'reactstrap'
...
import mb51 from '../../redux/actions/mb51'

const { REACT_APP_BACKEND_URL } = process.env

class ReportMB51 extends Component {
  constructor(props) {
    super(props)
    this.state = {
      sidebarOpen:   false,
  ...
  componentDidUpdate(prevProps) {
    const { isGenerate, isDelete, alertMsg } = this.props.mb51

    if (isGenerate !== prevProps.mb51.isGenerate && isGenerate !== null) {
      this.props.resetMb51()
      this.showConfirm(
        isGenerate ? 'success' : 'error',
        isGenerate ? 'Berhasil Generate Report MB51' : (alertMsg || 'Gagal Generate Report MB51')
      )
      this.getDataMb51()
    }

    if (isDelete !== prevProps.mb51.isDelete && isDelete !== null) {
      this.props.resetMb51()
      this.showConfirm(
  ...
  getDataMb51 = async () => {
    ...
    await this.props.getMb51(token, this.getPeriod(), this.state.search)
  }

  showConfirm = (type, msg) => {
    this.setState({ confirmType: type, confirmMsg: msg, modalConfirm: true })
    setTimeout(() => this.setState({ modalConfirm: false }), 3500)
  }

  // ─────────────────────────────────────────────
  // Event handlers
  // ─────────────────────────────────────────────
  handleMonth = (e) => this.setState({ selectMonth: e.target.value }, this.getDataMb51)
  handleYear  = (e) => this.setState({ selectYear:  e.target.value }, this.getDataMb51)

  onSearch = (e) => {
    this.setState({ search: e.target.value })
    if (e.key === 'Enter') this.getDataMb51()
  }
  ...
  reportToggleAll = () => {
    const { dataMb51 } = this.props.mb51
    const { listReport } = this.state
    this.setState({
      listReport: listReport.length === dataMb51.length ? [] : dataMb51.map(x => x.id)
    })
  }
  ...
  prosesGenerate = async () => {
    ...
    await this.props.generateMb51(token, {
  ...
  prosesDelete = async () => {
    ...
    await this.props.deleteMb51(token, { listId: this.state.listReport })
  }
  ...
  downloadFile = async (item) => {
    try {
      ...
      const res = await axios.get(
        `${REACT_APP_BACKEND_URL}/mb51/report/download/${item.id}`,
  ...
  // Render helpers
  // ─────────────────────────────────────────────
  getStatusLabel = (s) => ({ 0: 'Processing', 1: 'Success', 2: 'Failed' }[s] || '-')
  getStatusColor = (s) => ({ 0: 'warning',    1: 'success', 2: 'danger'  }[s] || 'secondary')

  render() {
    const { dataMb51, isLoading } = this.props.mb51
      ...
      <>
        <div className={styleTrans.app}>
          ...
          <div className={`${styleTrans.mainContent} ${sidebarOpen ? styleTrans.collapsedContent : ''}`}>
            <h2 className={styleTrans.pageTitle}>Report MB51</h2>
            ...
            {/* Table */}
            <table className={`${styleTrans.table} ${dataMb51.length > 0 ? styleTrans.tableFull : ''}`}>
              <thead>
                <tr>
                  <th>
                    <input
                      type="checkbox"
                      checked={listReport.length > 0 && listReport.length === dataMb51.length}
        ...
        <Modal isOpen={this.state.modalGenerate} toggle={this.toggleModalGenerate} centered size="md">
          <ModalBody>
            <div className="modalApprove">
              <div>
                <p>Generate Report MB51 untuk periode</p>
                <strong>{periodLabel}</strong>
                <p className="mt-2">
                  Proses ini menarik data dalam jumlah besar dari Redshift.
                  <br />
                  <span className="text-warning">⚠ Estimasi waktu: 15–60 menit.</span>
                  <br />
                  Lanjutkan?
                </p>
              </div>
        ...
        <Modal isOpen={this.state.modalDelete} toggle={this.toggleModalDelete} centered size="md">
          <ModalBody>
            <div className="modalApprove">
              <p>Anda yakin ingin menghapus <strong>{listReport.length}</strong> report MB51?</p>
              <div className="btnApproveIo">
...
const mapStateToProps = state => ({ mb51: state.mb51 })

const mapDispatchToProps = {
  getMb51:      mb51.getMb51,
  generateMb51: mb51.generateMb51,
  deleteMb51:   mb51.deleteMb51,
  resetMb51:    mb51.resetMb51,
}

export default connect(mapStateToProps, mapDispatchToProps)(ReportMB51)