...
class ReportInventory extends Component {
    ...
    getDataRepinv = async (value) => {
        const token = localStorage.getItem("token")
        const { page } = this.props.inventory
        const { selectMonth, typeReport } = this.state
        const year = moment().year(); // misalnya 2025
        const startOfMonth = moment(`${year}-${selectMonth}-01`, "YYYY-M-DD").startOf("month");
    ...
    render() {
        ...
        const startOfMonth = moment(`${year}-${this.state.selectMonth}-01`, "YYYY-M-DD").startOf("month")
        const dataStatus = [
                        ...
                        <div className="background-logo">
                            ...
                            <div className="bodyDashboard">
                                <div className="headMaster">
                                    <div className="titleDashboard col-md-12">Report Inventory</div>
                                </div>
                                <div className="secHeadDashboard">
                                    <div>
                                        ...
                                        <DropdownMenu>
                                        <DropdownItem className="item" onClick={() => this.getDataRepinv({limit: 10, search: ''})}>10</DropdownItem>
                                            <DropdownItem className="item" onClick={() => this.getDataRepinv({limit: 20, search: ''})}>20</DropdownItem>
                                            <DropdownItem className="item" onClick={() => this.getDataRepinv({limit: 50, search: ''})}>50</DropdownItem>
                                        </DropdownMenu>
                                        </ButtonDropdown>
                                ...
                                <div className="secEmail mt-4">
                                    <div className="rowCenter">
                                        ...
                                        {/* <Button className='ml-1' onClick={this.openModalAdd} color="warning" size="lg">Upload</Button> */}
                                            ...
                                            {stateInv.length > 0 && stateInv.map((item, index) => {
                                                return (
                                                    <tr>
                                                        ...
                                                        {dataType.map(type => {
                                                            return (
                                                                <td>{dataRepinv.length === 0 ? '-' : dataRepinv.find(x => (x.plant === item.plant && x.type === type)) === undefined ? '-' : `V - ${moment(dataRepinv.find(x => (x.plant === item.plant && x.type === type)).createdAt).format('DD/MM/YYYY')} - upload by ${dataRepinv.find(x => (x.plant === item.plant && x.type === type)).user_upload}`}</td>
                                                            )
                                                        })}
                ...
                <Modal toggle={this.openModalUpload} isOpen={this.state.modalUpload} >
                    ...
                    <ModalBody className="modalUpload">
                        ...
                        <div className="btnUpload">
                            ...
                            <Button color="primary" disabled={this.state.fileUpload === "" ? true : false } onClick={this.uploadRepinv}>Upload</Button>
